...
class QuranCtrl extends GetxController {
  ...
  /// Public accessor for quranRepository (used by extensions in other files)
  ...
  // PreloadPageController الداخلي (used by extensions)
  PreloadPageController? pageControllerInternal;
  ...
  Future<void> ensureCoreDataLoaded() async {
    ...
    QuranCtrl.instance.state.isTajweedEnabled.value =
        GetStorage().read(StorageConstants().isTajweed) ?? false;
  ...
  Future<void> _ensureQpcV4AssetsLoaded() async {
    ...
    if (qpcV4Store != null) return;

    _qpcV4LoadFuture ??= () async {
      try {
        qpcV4Store = await QpcV4AssetsLoader.load();
