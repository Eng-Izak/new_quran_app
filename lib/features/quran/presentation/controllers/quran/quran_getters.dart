...
extension QuranGetters on QuranCtrl {
  ...
  PreloadPageController get quranPagesController {
    ...
    QuranCtrl.instance.pageControllerInternal ??= PreloadPageController(
      initialPage: (quranRepository.getLastPage() ?? 1) - 1,
      keepPage: true,
      viewportFraction: 1.0,
    );
    return QuranCtrl.instance.pageControllerInternal!;
  }
  ...
  set quranPagesController(PreloadPageController controller) {
    ...
    if (QuranCtrl.instance.pageControllerInternal != null &&
        QuranCtrl.instance.pageControllerInternal!.hasClients) {
      try {
        QuranCtrl.instance.pageControllerInternal!.dispose();
      } catch (_) {
        // تجاهل الأخطاء إذا كان قد تم التخلص منه مسبقاً
      }
    }
    QuranCtrl.instance.pageControllerInternal = controller;
  }
  ...
  Future<void> selectRecitation(
    ...
    GetStorage().write(StorageConstants().fontsSelected, idx);
  ...
  Future<void> switchFontType({required int fontIndex}) async {
    ...
    final storageValue =
        GetStorage().read<bool>(StorageConstants().isDownloadedCodeV4Fonts);
  ...
  List<List<AyahModel>> getCurrentPageAyahsSeparatedForBasmalah(
          int pageIndex) {
    if (state.pages.isEmpty || pageIndex < 0 || pageIndex >= state.pages.length) {
      return [];
    }
  ...
  List<List<LineModel>> getCurrentPageAyahsSeparatedForBasmalahQcfV1AsLines(
      int pageIndex) {
    if (staticPages.isEmpty || pageIndex < 0 || pageIndex >= staticPages.length) {
      return [];
    }
  ...
  List<AyahModel> get currentPageAyahs {
    final pageIndex = state.currentPageNumber.value - 1;
    if (state.pages.isEmpty || pageIndex < 0 || pageIndex >= state.pages.length) {
      return [];
    }
    return state.pages[pageIndex];
  ...
  int getAyahUnqNumberByPageAndIndex(int page, int index) {
    ...
    final pageAyahs = state.pages[pageIndex];
    if (index < 0 || index >= pageAyahs.length) {
      return 1;
    }
    return pageAyahs[index].ayahUQNumber;
  }
  ...
  void setDisplayMode(QuranDisplayMode mode) {
    ...
    GetStorage().write(StorageConstants().displayMode, mode.storageIndex);
  ...
  void loadSavedDisplayMode() {
    final saved = GetStorage().read<int>(StorageConstants().displayMode);
    if (saved != null) {
