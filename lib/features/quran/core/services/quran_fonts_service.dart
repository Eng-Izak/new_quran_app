...
class QuranFontsService {
  ...
  static Future<Uint8List> _decompressFromAsset(int page) async {
    final path = _assetPath(page);
    ByteData data;
    try {
      data = await rootBundle.load(path);
    } catch (e) {
      if (path.startsWith('packages/quran_library/')) {
        final localPath = path.replaceFirst('packages/quran_library/', '');
        data = await rootBundle.load(localPath);
      } else {
        rethrow;
      }
    }
