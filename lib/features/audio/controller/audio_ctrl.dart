...
class AudioCtrl extends GetxController {
  ...
  Future<void> onInit() async {
    ...
    // await updateDownloadedAyahsMap();
    ...
    QuranCtrl.instance;
    if (!kIsWeb) {
      state.dirCache ??= await getApplicationDocumentsDirectory();
    }
    // loadLastSurahAndPosition();
    await Future.wait([
      _addDownloadedSurahToPlaylist(),
      updateDownloadedAyahsMap(),
    ]);
  ...
  Future<void> downloadSurah({int? surahNum}) async {
    final isConnected = sl<InternetConnectionCubit>().isConnected;
    ...
    if (await file.exists()) {
      ...
      if (!isConnected) {
        ...
        if (await _downloadFile(filePath, urlSurahFilePath)) {
          addFileAudioSourceToPlayList(filePath);
  ...
  Future<String> downloadFileIfNotExist(String url, String fileName,
      {BuildContext? context,
      bool showSnakbars = true,
      bool setDownloadingStatus = true,
      int? ayahUqNumber}) async {
    final isConnected = sl<InternetConnectionCubit>().isConnected;
    final isPhoneData = sl<InternetConnectionCubit>().isPhoneData;
    String path = p.join((await state.dir).path, fileName);
    var file = File(path);
  ...
  void enableSurahPositionSaving() {
    ...
    state.surahPositionSubscription?.cancel();

    // استخدام DateTime لتتبع آخر وقت حفظ فعلي
    DateTime? lastSaveTime;
    int? lastSavedPosition;

    state.surahPositionSubscription =
      ...
      if (state.isPlayingSurahsMode) {
        ...
        if (shouldSave) {
          state.box
              .write(AudioStorageConstants.lastPosition, positionInSeconds);
  ...
  void disableSurahPositionSaving() {
    state.surahPositionSubscription?.cancel();
    state.surahPositionSubscription = null;

    // حفظ الموضع والسورة الأخيرة قبل التعطيل
    if (state.isPlayingSurahsMode && state.lastPosition.value > 0) {
      state.box
          .write(AudioStorageConstants.lastPosition, state.lastPosition.value);
      saveLastSurahListen(state.currentAudioListSurahNum.value);
    }
  }
