...
extension AyahCtrlExtension on AudioCtrl {
  ...
  Future<void> _playSingleAyahFile(
    ...
    String? filePath;
    if (!kIsWeb) {
      filePath = p.join((await state.dir).path, currentAyahFileName);
    }
    ...
    try {
      ...
      if (kIsWeb) {
        ...
        await downloadFileIfNotExist(currentAyahUrl, currentAyahFileName,
    ...
    try {
      ...
      if (kIsWeb) {
        ...
        audioSources = List<AudioSource>.generate(
          batchCount,
          (i) => AudioSource.file(
            p.join(directory.path, ayahsFilesNames[initialIndex + i]),
      ...
      state.currentIndexSubscription =
  ...
  Future<bool> get validateBeforePlayAyah async {
    ...
    try {
      final dir = await state.dir;
      final filePath = p.join(dir.path, currentAyahFileName);
  ...
  Future<void> updateDownloadedAyahsMap() async {
    if (kIsWeb) {
  ...
  Future<void> startDownloadAyahSurah(int surahNumber,
    ...
    try {
      ...
      for (final ayah in ayahs) {
        ...
        await downloadFileIfNotExist(url, fileName,
