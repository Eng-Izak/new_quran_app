...
extension TafsirUi on TafsirCtrl {
  ...
  Future<void> handleRadioValueChanged(int val, {int? pageNumber}) async {
    ...
    box.write(StorageConstants().radioValue, val);
    if (!tafsirAndTranslationsItems[val].isTranslation) {
      box.write(StorageConstants().isTafsir, true);
      try {
        await fetchData(
        ...
        box.write(StorageConstants().radioValue, radioValue.value);
      }
    } else {
      String langCode = tafsirAndTranslationsItems[val].fileName;
      translationLangCode = langCode;
      await fetchTranslate();
      box.write(StorageConstants().translationLangCode, langCode);
      box.write(StorageConstants().isTafsir, false);
