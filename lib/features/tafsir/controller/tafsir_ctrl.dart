...
class TafsirCtrl extends GetxController {
  ...
  RxString progressString = RxString("0");
  ...
  Future<void> removeCustomTafsir(TafsirNameModel model) async {
    ...
    if (!kIsWeb &&
      ...
      final f = File(p.join(_appDir.path, model.databaseName));
  ...
  Future<void> initTafsir() async {
    ...
    tafsirAndTranslationsItems.assignAll(defaultTafsirList);
  ...
  Future<void> _loadSelectedDefaultTafseer() async {
    radioValue.value =
        box.read(StorageConstants().radioValue) ?? _defaultTafsirIndex;
    translationLangCode =
        box.read(StorageConstants().translationLangCode) ?? 'en';
    TafsirCtrl.instance.fontSizeArabic.value =
        box.read(StorageConstants().fontSize) ?? 20.0;
  }
      ...
      if (selectedTafsir.type == TafsirFileType.json) {
        ...
        if (selectedTafsir.fileName == _defaultDownloadedTafsirName) {
          ...
          if (kIsWeb) {
            ...
            String filePath = p.join(_appDir.path, selectedTafsir.databaseName);
  ...
  Future<void> fetchTranslate() async {
    try {
      ...
      if (radioValue.value == translationsStartIndex) {
        ...
        final String path = p.join(_appDir.path, '$translationLangCode.json');
  ...
  Future<void> tafsirAndTranslationDownload(int i) async {
    ...
    if (!selected.isTranslation) {
      path = p.join(_appDir.path, selected.databaseName);
      fileUrl = '$_ghTafsirAndTranslate/${selected.databaseName}';
    } else {
      path = p.join(_appDir.path, '${selected.fileName}.json');
  ...
  Future<bool> deleteTafsirOrTranslation({required int itemIndex}) async {
    ...
    try {
      ...
      if (!selected.isTranslation) {
        filePath = p.join(_appDir.path, selected.databaseName);
      } else {
        filePath = p.join(_appDir.path, '${selected.fileName}.json');
      }
